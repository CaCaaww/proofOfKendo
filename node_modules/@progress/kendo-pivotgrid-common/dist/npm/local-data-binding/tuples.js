...
exports.createLocalDataState = function (args) {
    ...
    var colTuples = columnAxes.length ?
        exports.createTuples(columnSettings, columnAxes, dataTree.get(fields.columnsData), sort, exclude) :
        measures.map(function (m) { return ({ members: [measureToMember(m)] }); });
    var columnTuples = columnAxes.length > 0 ? exports.addMultipleMeasures(colTuples, measures) : colTuples;
    var rowTuples = rowAxes.length ? exports.createTuples(rowSettings, rowAxes, dataTree, sort, exclude) : [{ members: [] }];
